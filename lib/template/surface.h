...
public:
	...
	// Constructor for using image source.
	Surface(unsigned char* Src, uint32_t Width, uint32_t Height, int Mode);

	~Surface();
	// operations
	void InitCharset();
	void SetChar( int c, const char* c1, const char* c2, const char* c3, const char* c4, const char* c5 );
	void PrintText( const char* t, int x1, int y1, uint c );
	void Clear( uint c );
	void Line( float x1, float y1, float x2, float y2, uint c );
	void Plot( int x, int y, uint c );
	void LoadFromFile( const char* file );

	// I extended the api to support to load from cstrings so I could obtain the width and height from a loaded image.
	void LoadFromBuffer(unsigned char* Src, uint32_t Width, uint32_t Height, int Mode);
