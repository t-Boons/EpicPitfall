...
void main()
{
	...
	// Close console if in distribute config.
#ifdef PF_DISTRIBUTE
	// Couldn't figure out how to close the window so here is the learning resource:
	// https://stackoverflow.com/questions/22271920/c-windows-how-to-close-a-console-window
	ShowWindow(GetConsoleWindow(), 0); //handle window
	FreeConsole();
#endif

	// Initialize random seed.
	InitSeed((uint)time(NULL));

	// initialize application
	InitRenderTarget( SCRWIDTH, SCRHEIGHT );
	Surface* screen = new Surface( SCRWIDTH, SCRHEIGHT );
	app = new Pitfall::Game();
	app->screen = screen;
	...
	// Bind main shader to game class for post processing and scaling.
	Pitfall::Game* pitfallApp = dynamic_cast<Pitfall::Game*>(app);
	Shader* shader = pitfallApp->InitializeShader();
	app->Init();
