...
Surface::Surface(unsigned char* Src, uint32_t Width, uint32_t Height, int Mode) : pixels(0), width(0), height(0)
{
	// check if file exists; show an error if there is a problem
	Surface::LoadFromBuffer(Src, Width, Height, Mode);
}
...
void Surface::LoadFromFile( const char* file )
{
	...
	LoadFromBuffer(data, width, height, n);

	// free stb_image data
	stbi_image_free( data );
}

void Tmpl8::Surface::LoadFromBuffer(unsigned char* src, uint32_t Width, uint32_t Height, int Mode)
{
	width = Width;
	height = Height;
	...
	if (Mode == 1) /* greyscale */ for (int i = 0; i < s; i++)
	{
	...
	{
		// Modified function so it saves the alpha chanel pixels.
		for (int i = 0; i < s; i++) pixels[i] = (src[i * Mode + 3] << 24) + (src[i * Mode + 0] << 16) + (src[i * Mode + 1] << 8) + src[i * Mode + 2];
	}
}
...
void Surface::PrintText( const char* s, int x1, int y1, uint c )
{
	if (!fontInitialized)
	{
